...
import { homeLead } from '../lib/editorial';
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="mb-16 max-w-3xl">
        <p className="mb-3 text-sm font-bold uppercase tracking-[0.24em] text-amber-700 dark:text-amber-500">{homeLead.eyebrow}</p>
        <h1 className="mb-6 text-4xl font-bold leading-tight text-stone-900 dark:text-stone-100 md:text-5xl">{homeLead.title}</h1>
        <p className="text-lg leading-relaxed text-stone-600 dark:text-stone-400 md:text-xl">{homeLead.subtitle}</p>
      ...
      <div className="mb-20 rounded-3xl border border-stone-200 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-900 md:p-8">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-2xl font-bold text-stone-900 dark:text-stone-100"><Activity className="text-amber-600 dark:text-amber-500" /> Latest fuel snapshot</h2>
            <p className="mt-1 text-sm text-stone-500 dark:text-stone-400">Every card below is tied to a source URL and a date stamp.</p>
          </div>
        </div>
        {featuredPrices.length ? (
          ...
          <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 p-6 dark:border-stone-700 dark:bg-stone-800/30 md:p-10">
            <p className="leading-relaxed text-stone-700 dark:text-stone-300">Waiting for the source-backed price file to load.</p>
          </div>
        )}
